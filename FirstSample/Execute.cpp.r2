...
{
#ifdef REDTEAM
	CameraVariables::cameraOffset.x = 500;
	CameraVariables::cameraOffset.y = 400;
#endif
#ifdef BLUETEAM
	CameraVariables::cameraOffset.x = -500;
	CameraVariables::cameraOffset.y = 400;
...
void rc17::Execute::getPillarCoor()
{
	...
	CameraVariables::cameraParam.pitch = 22;
		...
		if (regionsFound.size() > 0)
		{
			for (size_t i = 0; i < regionsFound.size(); ++i)
			{
				...
				if (SaucerCoordinate.z > 0)
				{
					...
					datafile << "飞盘编号: " << RegionDetector::regionVector[i].getSaucerIndex() << " X: " << setw(2) << SaucerCoordinate.x << " Y: " << setw(2) << SaucerCoordinate.y << " Z: " << SaucerCoordinate.z << endl;
				}
			}
		}
	}
